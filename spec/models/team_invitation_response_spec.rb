...
RSpec.describe TeamInvitationResponse, type: :model do
  ...
  describe "#save" do
    let(:invitation) { create(:team_invitation) }
    let(:user) { create(:user) }
    subject { TeamInvitationResponse.new(user: user, invitation: invitation, status: status) }
    before { subject }
    context "accept" do
      let(:status) { "accept" }
      it "destroys invitation" do
        expect{subject.save}.to change{TeamInvitation.count}.by(-1)
      end
